...
struct Color {
    ...
    explicit constexpr Color(const v3& v, float alpha) {
        rgb = v;
        a = alpha;
    }
    explicit constexpr Color(const Color& c, float alpha) {
        rgb = c.rgb;
        a = alpha;
    }
    explicit constexpr Color(float value) {
        r = g = b = value;
        a         = 1.0f;
    }
    explicit constexpr Color(float value, float alpha) {
        r = g = b = value;
        a         = alpha;
    }
    explicit constexpr Color(float r, float g, float b) : r(r), g(g), b(b) {
        a = 1.0f;
    }
    explicit constexpr Color(float r, float g, float b, float a) : r(r), g(g), b(b), a(a) {}
    explicit constexpr Color(uint32 num) {
        r = float((num >> 3 * 8) & 0xFF) / 255.0f;
        g = float((num >> 2 * 8) & 0xFF) / 255.0f;
        b = float((num >> 1 * 8) & 0xFF) / 255.0f;
        a = float((num >> 0 * 8) & 0xFF) / 255.0f;
    }
    explicit operator uint32() const {
        uint32 num = 0;
        num |= uint32(r * 255.0f) << (3 * 8);
        num |= uint32(g * 255.0f) << (2 * 8);
        num |= uint32(b * 255.0f) << (1 * 8);
        num |= uint32(a * 255.0f) << (0 * 8);
        return num;
    }
...
const Color spellbook_1 = Color::hsv(305.f + 15.f, 0.10f, 0.9f);
const Color spellbook_2 = Color::hsv(300.f + 15.f, 0.16f, 0.8f);
const Color spellbook_3 = Color::hsv(295.f + 15.f, 0.23f, 0.7f);
const Color spellbook_4 = Color::hsv(290.f + 15.f, 0.31f, 0.6f);
const Color spellbook_5 = Color::hsv(285.f + 15.f, 0.40f, 0.5f);
const Color spellbook_6 = Color::hsv(280.f + 15.f, 0.50f, 0.4f);
const Color spellbook_7 = Color::hsv(275.f + 15.f, 0.61f, 0.3f);
const Color spellbook_8 = Color::hsv(270.f + 15.f, 0.73f, 0.2f);
