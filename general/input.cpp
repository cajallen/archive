...
void Input::setup(GLFWwindow* input_window) {
    assert_else(input_window != nullptr)
        return;
    window = input_window;
...
void default_framebuffer_size_callback(GLFWwindow* window, int x, int y) {
    auto& stack = InputCallbackInfo<ResizeCallback>::stack();
    for (auto& callback_info : stack) {
        auto& [fp, prio, name, data] = callback_info;
        bool esc = (*fp)({window, x, y, data});
        if (esc)
